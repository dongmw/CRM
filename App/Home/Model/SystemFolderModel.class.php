...
class SystemFolderModel extends Model {
	...
	function get_authed_folder($user_id,$folder=null){
		...
		$folder_list=array();
		$list=$this->where("folder='$folder' and is_del=0")->getField('id,id');
	...
	function get_folder_auth($folder_id){				 
		...
		//}
		//return $result;			
		$result['admin'] = $this -> _check_auth($auth_list['admin']);
		$result['write'] = $this -> _check_auth($auth_list['write']);
		$result['read']  = $this -> _check_auth($auth_list['read']);
		if ($result['admin']==true){
			$result['write']=true;				
		}
		if ($result['write']==true){
	...
	private function _check_auth($auth_list){
			...
			foreach ($arrtmp as $item) {
				if (stripos($item, "dept_")!==false){
					...
					$emp_list=$emp_list['username'];
					
					if (in_array(getusername(),$emp_list)){
						return true;
					}
				} else {
					if (stripos($item,getusername())!==false){
						return true;
					}
				}
			}
	...
	private function get_emp_list_by_dept_id($id){
        $dept = tree_to_list(list_to_tree(M("auth_group")->where('status=1') -> select(), $id));//Dept
        $dept = rotate($dept);
		$dept=$dept['id'];
