...
class JxcoutboundreturnController extends CommonController{
	...
	public function _befor_add(){
		...
		$list.='<tr id="jxcoutboundreturn_add_0">
<th title="No." width="38"><input type="text" readonly="readonly" name="jxcoutboundreturn_add_group[0]" class="no textcenter" data-rule="required" value="" size="1"><input type="hidden" id="jxcoutboundreturn_add" name="jxcoutboundreturn_add" value="" size="1"></th>
...
<th title="购货单价"><input type="text" name="mpl_jg[0]" data-rule="required;number;length[~20]" onkeyup="jxc_xhthd_add_get_num_(\'jxcoutboundreturn_add_group\',$(this).attr(\'data-group\'),\'jxcoutboundreturn_add\')" id="jxc_xhthd_add_mpl_jg[0]" value="0" style="width:100%" data-group="jxcoutboundreturn_add_group[0]" onfocus="select()" class="textright"></th>
<th title="数量"><input type="text" name="mpl_sl[0]" data-rule="required;number;length[~20]" onkeyup="jxc_xhthd_add_get_num_(\'jxcoutboundreturn_add_group\',$(this).attr(\'data-group\'),\'jxcoutboundreturn_add\')" id="jxc_xhthd_add_mpl_sl[0]" value="0" style="width:100%" data-group="jxcoutboundreturn_add_group[0]" onfocus="select()" class="textright"></th>
<th title="金额"><input type="text" name="mpl_je[0]" data-rule="required;number;length[~20]" onkeyup="jxc_xhthd_add_get_num_(\'jxcoutboundreturn_add_group\',$(this).attr(\'data-group\'),\'jxcoutboundreturn_add\')" id="jxc_xhthd_add_mpl_je[0]" value="0" style="width:100%" data-group="jxcoutboundreturn_add_group[0]" onfocus="select()" class="textright"></th>
<th title="备注"><input type="text" name="mpl_bz[0]" data-rule="length[~50]" value="" style="width:100%"></th>
<th title="" data-addtool="true"><a href="'.U('public/flow','id='.$id.'&type=del').'" onclick="delay_time_run(\'jxc_xhthd_add_num_all(\\\'jxcoutboundreturn_add\\\')\', \'500\')" class="btn row-del"><i class="fa red fa-remove"></i></a></th>
</tr>
';
	...
	public function _befor_insert($data){
		...
		if (!empty($str_mp_id)){//采购订单ID
			...
			$strlen = 0;
			for ($i=0;$i<count(I('post.jxcoutboundreturn_add_group'));$i++) {
				if(strlen($mi_id[$i])>0){$strlen++;}
			}
			if($strlen==0){$this->mtReturn(300,L('_OPERATION_FAIL_').'至少添加一条销货退货记录！','',false);return;}
			
			$del=M("jxcoutboundreturnlist")->where("attid='".$data['attid']."'")->delete();
			...
			for ($i=0;$i<count(I('post.jxcoutboundreturn_add_group'));$i++) {
				...
				$model1 = M('jxcoutboundreturnlist');
			...
			$sql = "SELECT LPAD(count(*)+1,3,0) ms_ddbh FROM ".$tablePrefix."jxcoutboundreturn WHERE mp_ddrq=DATE_FORMAT('".$data['mp_ddrq']."','%Y-%m-%d')";
			$rs = $m->query($sql);
			if($rs){
				$data['ms_ddbh']='XSTH'.date("Ymd",strtotime($data['mp_ddrq'])).''.$rs[0]['ms_ddbh'];
			}else{
				$data['ms_ddbh']='XSTH'.date("Ymd",strtotime($data['mp_ddrq'])).''.'001';
			}
	...
	public function _befor_edit(){
		...
		$i=0;
		$list_str=M('jxcoutboundreturnlist')->where("attid='".$vo['attid']."' and status=1")->order('id')->select();
		foreach($list_str as $v){
			$list.='<tr id="jxcoutboundreturn_edit_'.$i.'">
<th title="No." width="38"><input type="text" readonly="readonly" name="jxcoutboundreturn_edit_group['.$i.']" class="no textcenter" data-rule="required" value="'.$i.'" size="1"><input type="hidden" id="jxcoutboundreturn_edit" name="jxcoutboundreturn_edit" value="'.$i.'" size="1"></th>
...
<th title="购货单价"><input type="text" name="mpl_jg['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_xhthd_edit_get_num_(\'jxcoutboundreturn_edit_group\',$(this).attr(\'data-group\'),\'jxcoutboundreturn_edit\')" id="jxc_xhthd_edit_mpl_jg['.$i.']" value="'.$v['mpl_jg'].'" style="width:100%" data-group="jxcoutboundreturn_edit_group['.$i.']" onfocus="select()" class="textright"></th>
<th title="数量"><input type="text" name="mpl_sl['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_xhthd_edit_get_num_(\'jxcoutboundreturn_edit_group\',$(this).attr(\'data-group\'),\'jxcoutboundreturn_edit\')" id="jxc_xhthd_edit_mpl_sl['.$i.']" value="'.$v['mpl_sl'].'" style="width:100%" data-group="jxcoutboundreturn_edit_group['.$i.']" onfocus="select()" class="textright"></th>
<th title="金额"><input type="text" name="mpl_je['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_xhthd_edit_get_num_(\'jxcoutboundreturn_edit_group\',$(this).attr(\'data-group\'),\'jxcoutboundreturn_edit\')" id="jxc_xhthd_edit_mpl_je['.$i.']" value="'.$v['mpl_je'].'" style="width:100%" data-group="jxcoutboundreturn_edit_group['.$i.']" onfocus="select()" class="textright"></th>
<th title="备注"><input type="text" name="mpl_bz['.$i.']" data-rule="length[~50]" value="'.$v['mpl_bz'].'" style="width:100%"></th>
<th title="" data-addtool="true"><a href="'.U('public/flow','id='.$id.'&type=del').'" onclick="delay_time_run(\'jxc_xhthd_edit_num_all(\\\'jxcoutboundreturn_edit\\\')\', \'500\')" class="btn row-del"><i class="fa red fa-remove"></i></a></th>
</tr>
';
			$i++;
		}
	...
	public function _befor_update($data){
		...
		$strlen = 0;
		for ($i=0;$i<count(I('post.jxcoutboundreturn_edit_group'));$i++) {
			if(strlen($mi_id[$i])>0){$strlen++;}
		}
		if($strlen==0){$this->mtReturn(300,L('_OPERATION_FAIL_').'至少添加一条销货退货记录！','',false);return;}
	...
	public function view(){
		...
		$i=0;
		$list_str=M('jxcoutboundreturnlist')->where("attid='".$attid."' and status=1")->order('id')->select();
		foreach($list_str as $v){
			$list.='<tr id="jxcoutboundreturn_edit_'.$i.'">
	...
	public function confirm(){
		...
		if($id){
			...
			if($data['status']==1 && $data['mp_shr']!=0){
				...
				//明细
				$attid=$data['attid'];
				$data1_list=M('jxcoutboundreturnlist')->where("attid='".$attid."' and status=1")->order('id')->select();
				foreach($data1_list as $data1){
					$mpl_id=$data1['mpl_id'];//销货退货订单ID
					...
					//更新分表审核状态
					$model3=M("jxcoutboundreturnlist");
	...
	*/
	public function make($type=1){
		...
		if(IS_POST){
			...
			$mw_id = I('post.mw_id');
			$mpl_sl = I('post.mpl_sl');
			$mpl_id = I('post.mpl_id');//销货退货单明细单
			//dump($mpl_id);
			//exit();
			$j=0;
			for ($i=0;$i<count(I('post.jxcoutboundreturn_make_group'));$i++) {
				
				if($mpl_id[$i]>0){
					//更新审核状态，标识为审核状态
					$m_u = M("jxcoutboundreturnlist");
					$m_u -> where('id='.$mpl_id[$i].'')->setField('mp_shr',getuserid());
				}
			...
			//exit();
			//if(count(I('post.jxcoutboundreturn_make_group')==$j){}
			...
			$list_str=M('jxcoutboundreturnlist')->where("attid='".$attid."' and status=1")->order('id')->select();
			foreach($list_str as $v){
				$list.='<tr id="jxcoutboundreturn_make_'.$i.'">
	<th title="No." width="38"><input type="text" readonly="readonly" name="jxcoutboundreturn_make_group['.$i.']" class="no textcenter" data-rule="required" value="'.$i.'" size="1"><input type="hidden" id="jxcoutboundreturn_make" name="jxcoutboundreturn_make" value="'.$i.'" size="1"><input type="hidden" name="mpl_id['.$i.']" value="'.$v['id'].'" size="1"></th>
	...
	<th title="仓库"><input type="text" readonly="readonly" name="mw_id['.$i.'].name" data-rule="required;length[~50]" value="'.get_table_field($v['mw_id'],'id','name','jxcwarehouse').'" style="width:100%" data-group="mw_id['.$i.']" data-toggle="lookup" data-url="index.php?m=home&c=public&a=jxcwarehouse"><input type="hidden" name="mw_id['.$i.'].id" value="'.$v['mw_id'].'"></th>
	<th title="购货单价"><input type="text" readonly="readonly" name="mpl_jg['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_xhthd_make_get_num_(\'jxcoutboundreturn_make_group\',$(this).attr(\'data-group\'),\'jxcoutboundreturn_make\')" id="jxc_xhthd_make_mpl_jg['.$i.']" value="'.$v['mpl_jg'].'" style="width:100%" data-group="jxcoutboundreturn_make_group['.$i.']" onfocus="select()" class="textright"></th>
	<th title="数量"><input type="text" readonly="readonly" name="mpl_sl['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_xhthd_make_get_num_(\'jxcoutboundreturn_make_group\',$(this).attr(\'data-group\'),\'jxcoutboundreturn_make\')" id="jxc_xhthd_make_mpl_sl['.$i.']" value="'.$v['mpl_sl'].'" style="width:100%" data-group="jxcoutboundreturn_make_group['.$i.']" onfocus="select()" class="textright"></th>
	<th title="金额"><input type="text" readonly="readonly" name="mpl_je['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_xhthd_make_get_num_(\'jxcoutboundreturn_make_group\',$(this).attr(\'data-group\'),\'jxcoutboundreturn_make\')" id="jxc_xhthd_make_mpl_je['.$i.']" value="'.$v['mpl_je'].'" style="width:100%" data-group="jxcoutboundreturn_make_group['.$i.']" onfocus="select()" class="textright"></th>
	<th title="备注"><input type="text" readonly="readonly" name="mpl_bz['.$i.']" data-rule="length[~50]" value="'.$v['mpl_bz'].'" style="width:100%"></th>
	</tr>
	';
	/*<th title="" data-addtool="true"><a href="'.U('public/flow','id='.$id.'&type=del').'" onclick="del_html_inc(jxcoutboundreturn_make_'.$i.')" class="btn row-del"><i class="fa red fa-remove"></i></a></th>*/
				$i++;
			}
	...
	public function del(){
		...
		if($id){
			...
			if($data['status']==1){
				$data['status']=0;
				$msg='无效';
				if (method_exists($this, '_befor_del')) {$this->_befor_del($id);}	
			}else{
				$data['status']=1;
				$msg='有效';
			}
			...
			foreach($info as $key=>$v){
				$attid=$v['attid'];
				$st['status']=0;
				M('jxcoutboundreturnlist')->where(array("attid"=>$attid))->save($st);
			...
			M('jxcoutboundreturnlist')->where('status=0')->delete();
