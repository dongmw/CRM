...
class JxcstoragereturnController extends CommonController{
	...
	public function _befor_add(){
		...
		$list.='<tr id="jxcstoragereturn_add_0">
<th title="No." width="38"><input type="text" readonly="readonly" name="jxcstoragereturn_add_group[0]" class="no textcenter" data-rule="required" value="" size="1"><input type="hidden" id="jxcstoragereturn_add" name="jxcstoragereturn_add" value="" size="1"></th>
...
<th title="购货单价"><input type="text" name="mpl_jg[0]" data-rule="required;number;length[~20]" onkeyup="jxc_ghthd_add_get_num_(\'jxcstoragereturn_add_group\',$(this).attr(\'data-group\'),\'jxcstoragereturn_add\')" id="jxc_ghthd_add_mpl_jg[0]" value="0" style="width:100%" data-group="jxcstoragereturn_add_group[0]" onfocus="select()" class="textright"></th>
<th title="数量"><input type="text" name="mpl_sl[0]" data-rule="required;number;length[~20]" onkeyup="jxc_ghthd_add_get_num_(\'jxcstoragereturn_add_group\',$(this).attr(\'data-group\'),\'jxcstoragereturn_add\')" id="jxc_ghthd_add_mpl_sl[0]" value="0" style="width:100%" data-group="jxcstoragereturn_add_group[0]" onfocus="select()" class="textright"></th>
<th title="金额"><input type="text" name="mpl_je[0]" data-rule="required;number;length[~20]" onkeyup="jxc_ghthd_add_get_num_(\'jxcstoragereturn_add_group\',$(this).attr(\'data-group\'),\'jxcstoragereturn_add\')" id="jxc_ghthd_add_mpl_je[0]" value="0" style="width:100%" data-group="jxcstoragereturn_add_group[0]" onfocus="select()" class="textright"></th>
<th title="备注"><input type="text" name="mpl_bz[0]" data-rule="length[~50]" value="" style="width:100%"></th>
<th title="" data-addtool="true"><a href="'.U('public/flow','id='.$id.'&type=del').'" onclick="delay_time_run(\'jxc_ghthd_add_num_all(\\\'jxcstoragereturn_add\\\')\', \'500\')" class="btn row-del"><i class="fa red fa-remove"></i></a></th>
</tr>
';
	...
	public function _befor_insert($data){
		...
		if (!empty($str_mp_id)){//采购订单ID
			...
			$strlen = 0;
			for ($i=0;$i<count(I('post.jxcstoragereturn_add_group'));$i++) {
				if(strlen($mi_id[$i])>0){$strlen++;}
			}
			if($strlen==0){$this->mtReturn(300,L('_OPERATION_FAIL_').'至少添加一条购货记录！','',false);return;}
			
			$del=M("jxcstoragereturnlist")->where("attid='".$data['attid']."'")->delete();
			...
			$sql = "SELECT LPAD(count(*)+1,3,0) ms_ddbh FROM ".$tablePrefix."jxcstoragereturn WHERE mp_ddrq=DATE_FORMAT('".$data['mp_ddrq']."','%Y-%m-%d')";
			$rs = $m->query($sql);
			if($rs){
				$data['ms_ddbh']='CGTH'.date("Ymd",strtotime($data['mp_ddrq'])).''.$rs[0]['ms_ddbh'];
			}else{
				$data['ms_ddbh']='CGTH'.date("Ymd",strtotime($data['mp_ddrq'])).''.'001';
			}
			...
			if($ms_bcfk==0){
				$data['mp_ddzt']= 113;//未退款
			}else if($ms_bcqk<=0){
				$data['mp_ddzt']=114;//全部退款
			}else{
				$data['mp_ddzt']=115;//部分退款
			}
		...
		//添加购货退货单
			...
			if($strlen==0){$this->mtReturn(300,L('_OPERATION_FAIL_').'至少添加一条购货退货记录！','',false);return;}
			if($strlen_mw!=count(I('post.jxcstoragereturn_add_group'))){$this->mtReturn(300,L('_OPERATION_FAIL_').'仓库：'.$strerr.'不存在或不可用！','',false);return;}
	...
	public function _befor_edit(){
		...
		$i=0;
		$list_str=M('jxcstoragereturnlist')->where("attid='".$vo['attid']."' and status=1")->order('id')->select();
		foreach($list_str as $v){
			$list.='<tr id="jxcstoragereturn_edit_'.$i.'">
<th title="No." width="38"><input type="text" readonly="readonly" name="jxcstoragereturn_edit_group['.$i.']" class="no textcenter" data-rule="required" value="'.$i.'" size="1"><input type="hidden" id="jxcstoragereturn_edit" name="jxcstoragereturn_edit" value="'.$i.'" size="1"></th>
...
<th title="购货单价"><input type="text" name="mpl_jg['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_ghthd_edit_get_num_(\'jxcstoragereturn_edit_group\',$(this).attr(\'data-group\'),\'jxcstoragereturn_edit\')" id="jxc_ghthd_edit_mpl_jg['.$i.']" value="'.$v['mpl_jg'].'" style="width:100%" data-group="jxcstoragereturn_edit_group['.$i.']" onfocus="select()" class="textright"></th>
<th title="数量"><input type="text" name="mpl_sl['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_ghthd_edit_get_num_(\'jxcstoragereturn_edit_group\',$(this).attr(\'data-group\'),\'jxcstoragereturn_edit\')" id="jxc_ghthd_edit_mpl_sl['.$i.']" value="'.$v['mpl_sl'].'" style="width:100%" data-group="jxcstoragereturn_edit_group['.$i.']" onfocus="select()" class="textright"></th>
<th title="金额"><input type="text" name="mpl_je['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_ghthd_edit_get_num_(\'jxcstoragereturn_edit_group\',$(this).attr(\'data-group\'),\'jxcstoragereturn_edit\')" id="jxc_ghthd_edit_mpl_je['.$i.']" value="'.$v['mpl_je'].'" style="width:100%" data-group="jxcstoragereturn_edit_group['.$i.']" onfocus="select()" class="textright"></th>
<th title="备注"><input type="text" name="mpl_bz['.$i.']" data-rule="length[~50]" value="'.$v['mpl_bz'].'" style="width:100%"></th>
<th title="" data-addtool="true"><a href="'.U('public/flow','id='.$id.'&type=del').'" onclick="delay_time_run(\'jxc_ghthd_edit_num_all(\\\'jxcstoragereturn_edit\\\')\', \'500\')" class="btn row-del"><i class="fa red fa-remove"></i></a></th>
</tr>
';
			$i++;
		}
	...
	public function _befor_update($data){
		...
		$strlen_mw = 0;
		$strerr = '';
		for ($i=0;$i<count(I('post.jxcstoragereturn_edit_group'));$i++) {
			if(strlen($mi_id[$i])>0){$strlen++;}
		...
		if($strlen==0){$this->mtReturn(300,L('_OPERATION_FAIL_').'至少添加一条购货退货记录！','',false);return;}
		if($strlen_mw!=count(I('post.jxcstoragereturn_edit_group'))){$this->mtReturn(300,L('_OPERATION_FAIL_').'仓库：'.$strerr.'不存在或不可用！','',false);return;}
	...
	public function view(){
		...
		$i=0;
		$list_str=M('jxcstoragereturnlist')->where("attid='".$attid."' and status=1")->order('id')->select();
		foreach($list_str as $v){
			$list.='<tr id="jxcstoragereturn_add_'.$i.'">
		...
		if($id){
			...
			if($data['status']==1 && $data['mp_shr']!=0){
				...
				$model->save($data);
					...
					//更新账户明细表
					M('jxcaccountlist')->where("ywtype=98 and number='".$data['ms_ddbh']."'")->delete();
					//更新账户明细表
				...
				//明细
				$attid=$data['attid'];
				$data1_list=M('jxcstoragereturnlist')->where("attid='".$attid."' and status=1")->order('id')->select();
	...
	*/
	public function make($type=1){
		...
		if(IS_POST){
			...
			$strlen_mw = 0;
			$strerr = '';
			for ($i=0;$i<count(I('post.jxcstoragereturn_make_group'));$i++) {
				if(strlen($mi_id[$i])>0){$strlen++;}
			...
			if($strlen==0){$this->mtReturn(300,L('_OPERATION_FAIL_').'至少一条购货退货记录！','',false);return;}
			if($strlen_mw!=count(I('post.jxcstoragereturn_make_group'))){$this->mtReturn(300,L('_OPERATION_FAIL_').'仓库：'.$strerr.'不存在或不可用！','',false);return;}
			//判断是否为空，是否有不存在的仓库
			...
			//exit();
			//if(count(I('post.jxcstoragereturn_make_group')==$j){}
			...
			$list_str=M('jxcstoragereturnlist')->where("attid='".$attid."' and status=1")->order('id')->select();
			foreach($list_str as $v){
				$list.='<tr id="jxcstoragereturn_make_'.$i.'">
	<th title="No." width="38"><input type="text" readonly="readonly" name="jxcstoragereturn_make_group['.$i.']" class="no textcenter" data-rule="required" value="'.$i.'" size="1"><input type="hidden" id="jxcstoragereturn_make" name="jxcstoragereturn_make" value="'.$i.'" size="1"><input type="hidden" name="mpl_id['.$i.']" value="'.$v['id'].'" size="1"></th>
	...
	<th title="仓库"><input type="text" readonly="readonly" name="mw_id['.$i.'].name" data-rule="required;length[~50]" value="'.get_table_field($v['mw_id'],'id','name','jxcwarehouse').'" style="width:100%" data-group="mw_id['.$i.']" data-toggle="lookup" data-url="index.php?m=home&c=public&a=jxcwarehouse"><input type="hidden" name="mw_id['.$i.'].id" value="'.$v['mw_id'].'"></th>
	<th title="购货单价"><input type="text" readonly="readonly" name="mpl_jg['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_ghthd_make_get_num_(\'jxcstoragereturn_make_group\',$(this).attr(\'data-group\'),\'jxcstoragereturn_make\')" id="jxc_ghthd_add_mpl_jg['.$i.']" value="'.$v['mpl_jg'].'" style="width:100%" data-group="jxcstoragereturn_make_group['.$i.']" onfocus="select()" class="textright"></th>
	<th title="数量"><input type="text" readonly="readonly" name="mpl_sl['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_ghthd_make_get_num_(\'jxcstoragereturn_make_group\',$(this).attr(\'data-group\'),\'jxcstoragereturn_make\')" id="jxc_ghthd_add_mpl_sl['.$i.']" value="'.$v['mpl_sl'].'" style="width:100%" data-group="jxcstoragereturn_make_group['.$i.']" onfocus="select()" class="textright"></th>
	<th title="金额"><input type="text" readonly="readonly" name="mpl_je['.$i.']" data-rule="required;number;length[~20]" onkeyup="jxc_ghthd_make_get_num_(\'jxcstoragereturn_make_group\',$(this).attr(\'data-group\'),\'jxcstoragereturn_make\')" id="jxc_ghthd_add_mpl_je['.$i.']" value="'.$v['mpl_je'].'" style="width:100%" data-group="jxcstoragereturn_make_group['.$i.']" onfocus="select()" class="textright"></th>
	<th title="备注"><input type="text" readonly="readonly" name="mpl_bz['.$i.']" data-rule="length[~50]" value="'.$v['mpl_bz'].'" style="width:100%"></th>
	</tr>
	';
	/*<th title="" data-addtool="true"><a href="'.U('public/flow','id='.$id.'&type=del').'" onclick="del_html_inc(jxcstoragereturn_make_'.$i.')" class="btn row-del"><i class="fa red fa-remove"></i></a></th>*/
				$i++;
			}
	...
	public function del(){
		...
		if($id){
			...
			foreach($info as $key=>$v){
				$attid=$v['attid'];
				$st['status']=0;
				M('jxcstoragereturnlist')->where(array("attid"=>$attid))->save($st);
			...
			M('jxcstoragereturnlist')->where('status=0')->delete();
